...
## Q20：为什么豆包等软件里技能重复显示？怎么彻底解决？

**根因**：部分软件（豆包、Trae 等）会**同时扫描多个已注册的技能根目录**（豆包的实际
扫描列表见下方 2026-09 实测注）。如果这些根全部通过 Junction 指向共享库，每个技能
就被加载多份，界面出现大量重复技能。

**这不是"同名不同内容"冲突，而是同一个技能被同一个软件从多个入口读到**。
脚本已在接入前检测并默认阻止（`duplicate-guard.ps1`），但已接入的根需要归并。

**一键根治**：

```powershell
.\scripts\merge-agent-roots.ps1        # 向导主菜单 [4] → [c] 也可
```

归并 = 每个软件只保留 **1 个**技能根接入共享库，其余根**只拆联接、重建为空目录**
（技能数据 100% 保留在共享库，未删除任何内容）。重启软件后重复立即消失。

- 归并后 `verify.ps1` 对空根显示 `[归并·空]`，不会误报。
- 若归并后某软件技能列表变空/变少 → 它实际读的是被归并的根，重新跑归并换保留那个根即可。
- 默认保留 `.agents\skills`（Codex / Cursor / Zed / Copilot / Doubao / Trae 共用），
  可覆盖最多软件；详见 `docs/04` 第 6b 节。

> **重要（2026-09 实测）**：豆包客户端的技能扫描列表（`agent_workspace.local_skill_paths`）
> 由豆包**自动发现**（当前为 `Doubao\skills` + `.agents\skills` 两项），**不读 `.user_skills`**。
> 归并后 `Doubao\skills` 虽已是空目录，**仍留在豆包的扫描列表里**——此时若该目录曾被
> Junction 指向共享库（或你之后在豆包 UI 里又安装了技能），豆包界面会再次出现"第三方技能
> 全部 ×2"的重复。
>
> **根治**：把空壳根目录**改名保留**（而非删除），豆包下次扫描即跳过该路径，只剩一个有效根：
>
> ```powershell
> Rename-Item "C:\Users\24664\Doubao\skills" "skills.empty-backup"   # 目录可随时改回
> # 若该根已从 agents.json 移除，重跑 .\scripts\verify.ps1 应全部 [OK]/[归并·空]
> ```
>
> 改名后**重启豆包**（新会话重新发现扫描列表）。同理适用于 Trae 等同样自管扫描列表的软件。

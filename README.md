...
| 🧭 **技能管理** | 技能按 **9 大分类**自动归类；中文简介；GitHub / skills.sh 仓库链接一键安装；移除双重确认；分类浏览；**技能介绍翻译**（英文简介批量译成中文）；**一键生成总路由技能（skill-router）**，在任何 Agent 中按情境推荐共享库技能 | 向导 [3] |
...
| [3] 技能管理 | **a** 技能清单（按 9 大分类分组，含中文简介）· **b** 从 GitHub / skills.sh 仓库链接安装 · **c** 移除（双重确认）· **d** 按分类浏览 · **e** 生成/刷新总路由技能（skill-router）· **f** 技能介绍翻译（英文简介译成中文写入 `_meta.json`） |
...
**生成总路由技能（skill-router，任何 Agent 都能按情境推荐技能）：**

```powershell
# 扫描共享库 → 按 9 大分类生成 <共享库>\router-guide\SKILL.md
.\scripts\generate-router-skill.ps1
```

生成后，你在任意 Agent 中触发 **skill-router**：直接描述你想做的事，
它会从共享库推荐最匹配的 1~3 个技能并说明理由；也可以直接点名技能名查用途。
向导 [3] → **e** 可随时重新生成（每次新增/移除技能后刷新一次即可）。
共享库内**所有技能一律收录**进路由索引，不做内容筛选（成人向、逆向破解、社工等全部技能都会出现在分类索引里）。
...
└── scripts/
    ...
    ├── generate-router-skill.ps1   # 生成/刷新总路由技能 skill-router（按 9 大分类 + 情境推荐共享库技能）
    ├── translate-skill-intro.ps1   # 技能介绍翻译：把英文技能简介译成中文写入 _meta.json（向导 [3]f）
    ...
    └── lib\
        ...
        └── translate.ps1           # 技能简介翻译 + frontmatter 解析（install-skill / translate-skill-intro 共用）
```
